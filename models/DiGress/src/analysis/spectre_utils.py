...
class SpectreSamplingMetrics(nn.Module):
    ...
    def forward(self, generated_graphs: list, name=None, current_epoch=0, val_counter=0, local_rank=0, constraint_config=None, test=False):
        ...
        if 'constr_val' in self.metrics_list:
            if local_rank == 0:
                print("Computing constraint validity...")
            from project_bisection import satisfies
            cval = 0.
            for X, A in generated_graphs:
                cval += satisfies(X, torch.tensor(A[None, :, :]), constraint_config=constraint_config).item()
            to_log['constr_val'] = cval/len(generated_graphs)
...
class Comm20SamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, datamodule, metrics_list=['degree', 'clustering', 'orbit']):
...
class PlanarSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, datamodule, metrics_list=['degree', 'clustering', 'orbit', 'spectre', 'planar']):
        super().__init__(datamodule=datamodule,
                         compute_emd=False,
                         metrics_list=metrics_list)
...
class SBMSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, datamodule, metrics_list=['degree', 'clustering', 'orbit', 'spectre', 'sbm']):
        super().__init__(datamodule=datamodule,
                         compute_emd=False,
                         metrics_list=metrics_list)
