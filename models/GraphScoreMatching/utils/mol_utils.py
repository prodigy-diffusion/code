...
def load_smiles(dataset='QM9', file_ext=''):
    import time
    if dataset == 'QM9':
        col = 'SMILES1'
    ...
    df = pd.read_csv(f'data/{dataset.lower()}{file_ext}.csv')
    ...
    train_data = list(df.loc[train_idx, col])
    test_data = list(df.loc[test_idx, col])

    return train_data, test_data


def only_valid_mols (x, adj, dataset, largest_connected_comp=True):
...
def gen_mol(x, adj, dataset, to_correct=True, largest_connected_comp=True):    
    ...
    for x_elem, adj_elem in zip(x, adj):
        mol = construct_mol(x_elem, adj_elem, atomic_num_list)
        cmol, no_correct = correct_mol (mol) if to_correct else (mol, True)
...
def print_graph (G):
    print ([(n, G.nodes()[n]['label']) for n in G.nodes()], 
           [(e, G.edges()[e]['label']) for e in G.edges()])