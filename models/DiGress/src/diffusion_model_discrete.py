...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def test_epoch_end(self, outs) -> None:
        ...
        log_dir = f"/nethome/ksharma323/ConstrGen_Diff/DiGress/generated_samples/{self.cfg['dataset']['name']}"
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        log_name = "generated_samples1"
        filename = f'{log_dir}/{log_name}.txt'
        # for i in range(2, 10):
        #     if os.path.exists(filename):
        #         filename = f'{log_dir}/{log_name}_generated_samples{i}.txt'
        #     else:
        #         break
    ...
    def sample_batch(self, batch_id: int, batch_size: int, keep_chain: int, number_chain_steps: int,
        ...
        for s_int in reversed(range(0, self.T)):
            ...
            # Sample z_s
            sampled_s, discrete_sampled_s, predicted_graph = self.sample_p_zs_given_zt(s_norm, t_norm, X, E, y, node_mask,
                                                                                       last_step=s_int == 100)
            ...
            from project_bisection import drifted_project, quantize, quantize_mol
            if self.cfg['dataset']['name'] in ['qm9', 'moses']:
                non_aromatic_mol = torch.all(E[:, :, :, -1].reshape(E.shape[0], -1) == 0, dim=1) # non-aromatic molecule
                A_E = 0 * E[non_aromatic_mol, :, :, 0] + 1 * E[non_aromatic_mol, :, :, 1] + 2 * E[non_aromatic_mol, :, :, 2] + 3 * E[non_aromatic_mol, :, :, 3]
                X_p = X[non_aromatic_mol, :, :]
                X_p, A_E = drifted_project(X_p.float(), A_E.float(), self.constr_config, s_int, self.T)
                X_p = F.one_hot(X_p.argmax(dim=-1), num_classes=X.shape[-1])
                X[non_aromatic_mol] = X_p
                A_E = quantize_mol(A_E)
                E[non_aromatic_mol] = F.one_hot(A_E, num_classes=E.shape[-1])
            else:
                A_E = E[:, :, :, 1].float()
                X, A_E = drifted_project(X.float(), A_E, self.constr_config, s_int, self.T)
                A_E = quantize(A_E)
                E[:, :, :, 1] = A_E
                E[:, :, :, 0] = 1 - A_E
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask, last_step: bool):
