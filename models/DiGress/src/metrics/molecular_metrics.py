...
class SamplingMolecularMetrics(nn.Module):
    ...
    def forward(self, molecules: list, name=None, current_epoch=0, val_counter=0, local_rank=0, constraint_config=None, filename=None, test=False):
        try:
            molecules = [(torch.tensor(X), torch.tensor(E)) for X, E in molecules]
        except:
            pass
        ...
        if test and local_rank == 0:
            if filename is not None:
                with open(f'{filename[:-4]}_smiles.txt', 'w') as fp:
        ...
        if constraint_config is not None:
            ...
            from src.project_bisection import satisfies
            cval = 0.
            for X, A in molecules:
                Xone = torch.zeros(X.shape[0], len(self.dataset_info.atom_decoder))
                Xone[torch.arange(X.shape[0]), X] = 1
                cval += satisfies(Xone[None, :, :], A[None, :, :], constraint_config=constraint_config).item()
            to_log['constr_val'] = cval/len(molecules)
