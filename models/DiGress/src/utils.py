...
def setup_wandb(cfg):
    ...
    kwargs = {'name': cfg.general.name, 'project': f'graph_ddm_{cfg.dataset.name}', 'config': config_dict,
              'settings': wandb.Settings(_disable_stats=True), 'reinit': True, 'mode': 'disabled'} #cfg.general.wandb}
    wandb.init(**kwargs)
    wandb.save('*.txt')
    ...
    with open (filename, 'r') as f:
        for line in f:
            ...
            if 'X:' in txt:
                ...
                X = [int(x) for x in txt.split()]
                to_X = False
            elif to_E:
                E.append([int(x) for x in txt.split()])
    return Xs, Es
